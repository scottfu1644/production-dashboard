...
(function ($, doc, win, Snap) {
    $.fn.scottDonut = function (apt,options) {
        this.appendTo(apt);
        var donuts = [];
        this.each(function () {
            donuts.push(new Donut(this, options));
        });
        if (donuts.length === 1) return donuts[0];
        return donuts;
    };

    // constructor
    var Donut = function (el, opts) {
        this.$el = $(el);
        this.opts = opts;
        this.snap = Snap(el);
    ...
    Donut.prototype = {
         ...
         ***********************************************************/
        buildArcPath: function (center, radius, degreesStart, degreesEnd) {
         ...
         ***********************************************************/
        animateArcByDegrees: function (arc, duration, callback) {
            var s = this.snap,
                t = this;
         ...
         ***********************************************************/
        animateArcByStroke: function (arc, strokeWidthStart, strokeWidthEnd, duration, callback) {
            var s = this.snap,
                t = this;
         ...
         ***********************************************************/
        animateShowLegendCircle: function (c, duration) {
         ...
         ***********************************************************/
        startShowAnimationOneEntry: function (d, percentStart, percentEnd, center, radius, callback) {
            d.arc = {
         ...
         ***********************************************************/
        translatedPoint: function (center, radius, degrees) {
         ...
         ***********************************************************/
        initInteraction: function (d, step) {
            var s = this.snap,
                t = this,
                l = d.legend,
                a = d.arc;
         ...
         ***********************************************************/
        startShowAnimation: function (index, totalPct, callback) {
            if (index && !totalPct) {
                callback = index;
                index = totalPct;
            }
