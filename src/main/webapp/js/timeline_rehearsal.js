...
//            background.css({'width':'100%','height':'99%','position':'absolute'});
            ...
            var statusSvg = this._jqSvg('svg','status_svg');
            statusSvg.css({width:'100%',height:'100%'});
            statusSvg.each(function(){
                _this._snapStatus = Snap(this);
            });
            $.each(_this.centerY,function(key,obj){
                ...
                _this._animateCircle(_this._snapStatus,80,obj.y,(heightUnit / 2.3),888,mina.linear,_fill);
            });
...
//            _getDateForInterval(x);
            ...
            return element;
        },_animateCircle : function(svg,x,y,r,duration,minaeff,fill,callback) {
            var circle = svg.circle(x,y,0).attr({fill: "#fff"})
                .animate({fill: fill, r:r},duration,minaeff,function(){
            ...
            return svg;
        }, _snapSvg : function(svg) {
            return Snap(svg);
        }, _jqSvg : function(tagName, id, style) {
            var _jqSvg = $('<'+tagName+' xmlns="http://www.w3.org/2000/svg"></'+tagName+'>');
            _jqSvg.attr('id',id);
            if(style) {
                _jqSvg.css(style);
            }
